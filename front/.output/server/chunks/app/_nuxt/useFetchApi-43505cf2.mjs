import { q as defu, r as hash } from '../../nitro/node-server.mjs';
import { computed, unref, reactive, ref, shallowRef, toRef, getCurrentInstance, onServerPrefetch } from 'vue';
import { f as fetchDefaults, e as useRuntimeConfig, b as asyncDataDefaults, d as useNuxtApp, c as createError } from '../server.mjs';
...
function useFetchApi(url, options = {}) {
  ...
  const defaults = {
    baseURL: (_a = config.baseUrl) != null ? _a : "http://localhost:8080",
    mode: "cors",
    redirect: "follow",
    cache: "no-store",
    onRequest({ request, options: options2 }) {
      options2.headers = { ...options2.headers, "Access-Control-Allow-Origin": "*" };
    },
    onResponse({ request, response, options: options2 }) {
    },
    onResponseError({ request, response, options: options2 }) {
    }
  };
  const params = defu(options, defaults);
  return useFetch(url, params, "$1YV9rb3tZJ");
}

export { useFetchApi as u };
//# sourceMappingURL=useFetchApi-43505cf2.mjs.map
